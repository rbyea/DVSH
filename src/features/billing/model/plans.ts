...
export type BillingPlanIncludeItem = {
  text: string;
  isNew?: boolean;
};

/** Что входит в любой тариф — набор функций один, отличается только срок. */
export const billingPlanIncludeGroups: Array<{
  title: string;
  items: BillingPlanIncludeItem[];
}> = [
  {
    title: 'Учёт на станции',
    items: [
      { text: 'Регламент ТО: свои позиции и график по пробегу', isNew: true },
      { text: 'Загрузка клиентов из Excel', isNew: true },
      { text: 'Заказ-наряды без лимита: приёмка, статусы, поиск' },
      { text: 'Клиенты и несколько авто на клиента — госномер, VIN или шасси, пробег' },
      { text: 'Работы: цена, часы, доп. работы, назначение мастера' },
      { text: 'Диагностика сканера: CSV с кодами ошибок, сверка VIN, карточка авто' },
    ],
  },
  {
    title: 'Клиенту',
    items: [
      { text: 'График ТО на публичной карточке', isNew: true },
      { text: 'Публичная ссылка без приложения и регистрации' },
      { text: 'Согласование работ и подтверждение после выдачи' },
      { text: 'История ремонтов по автомобилю' },
      { text: 'Диагностика сканера на карточке авто' },
      { text: 'Уведомления о статусе в MAX' },
    ],
  },
  {
    title: 'Станция',
    items: [
      { text: 'Справочник мастеров' },
      { text: 'Сводка выполненных работ и доля мастера' },
      { text: 'Профиль станции: название, телефон, адрес, график' },
    ],
  },
];
